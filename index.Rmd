...
```{r Sonia, fig.cap='Effective Transfer Entropy for Change SONIA'}
plot_data<-rename_data(res_SONIA)
ggplot(plot_data, aes(x = ticker, y = ete)) + 
  facet_wrap(~dir) +
  geom_hline(yintercept = 0, color = "gray") +
  theme(axis.text.x = element_text(angle = 90)) +
...
```{r EUce, fig.cap='Effective Transfer Entropy for Change EU Libor'}
...
```{r USce,fig.cap='Effective Transfer Entropy for Change US Libor'}
...
```{r Soniace, fig.cap='Effective Transfer Entropy for Change SONIA'}
plot_data<-rename_data(res_SONIA_CE)
ggplot(plot_data, aes(x = ticker, y = ete)) + 
  facet_wrap(~dir) +
  geom_hline(yintercept = 0, color = "gray") +
  theme(axis.text.x = element_text(angle = 90)) +
